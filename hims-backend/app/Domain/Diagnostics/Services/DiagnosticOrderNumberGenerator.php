...
namespace App\Domain\Diagnostics\Services;
...
class DiagnosticOrderNumberGenerator
{
    public function nextForHospital(Hospital $hospital): string
    {
        return DB::transaction(function () use ($hospital): string {
            ...
            $counter = SequenceCounter::query()
                ...
                ->where('branch_id', 0)
                ->where('name', 'diagnostic_order_number')
                ->lockForUpdate()
                ->firstOrFail();
            ...
            return sprintf('%s-DX-%06d', $prefix, $counter->current_value);
        });
    }
}
